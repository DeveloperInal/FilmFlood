...
interface SerialCategoryDetailsProps {
    categories: Category[]
...
export default function SerialCategoryDetails({
                                                ...
                                                selectedCategory,
                                                serials,
                                            }: SerialCategoryDetailsProps) {
    return (
        <div className="flex-1 bg-gray-800 text-foreground min-h-screen p-4 md:p-6">
            <h1 className="text-2xl md:text-3xl font-bold mb-6 text-center text-white">Выберите категорию сериалов</h1>
                    ...
                    {selectedCategory ? (
                        <div>
                            <h2 className="text-xl font-bold mb-4 text-white">
                                Сериалы в категории: {categories.find((c) => c.href === selectedCategory)?.name}
                            </h2>
                            <ScrollArea className="h-[calc(100vh-300px)] md:h-[calc(100vh-200px)] w-full rounded-lg border border-gray-700 p-4 bg-gray-900">
                                {serials && serials.length > 0 ? (
                                    ...
                                    <div className="flex items-center justify-center h-full">
                                        <p className="text-gray-400 text-base">Нет сериалов в данной категории</p>
                                    </div>
                                )}
                            </ScrollArea>
                        </div>
                        ...
                        <div className="flex items-center justify-center h-full">
                            <p className="text-gray-400 text-lg">Выберите категорию для просмотра сериалов</p>
                        </div>
                    )}
                </div>
            </div>
